...
type
  ...
  TOldFMXWorkScheduler = class(TComponent)
    protected
...
var
  GlobalOldFMXWorkScheduler : TOldFMXWorkScheduler = nil;
...
constructor TOldFMXWorkScheduler.Create(AOwner: TComponent);
begin
...
destructor TOldFMXWorkScheduler.Destroy;
begin
  DestroyThread;
...
procedure TOldFMXWorkScheduler.AfterConstruction;
begin
...
procedure TOldFMXWorkScheduler.CreateThread;
begin
...
procedure TOldFMXWorkScheduler.DestroyThread;
begin
  try
    if (FThread <> nil) then
      begin
...
procedure TOldFMXWorkScheduler.DoMessageLoopWork;
begin
  if (GlobalOldCEFApp <> nil) then GlobalOldCEFApp.DoMessageLoopWork;
end;

procedure TOldFMXWorkScheduler.SetDefaultInterval(aValue : integer);
begin
...
{$IFDEF MSWINDOWS}
{$WARN SYMBOL_PLATFORM OFF}
procedure TOldFMXWorkScheduler.SetPriority(aValue : TThreadPriority);
begin
  FPriority := aValue;
...
procedure TOldFMXWorkScheduler.DepleteWork;
var
  i : cardinal;
begin
...
procedure TOldFMXWorkScheduler.ScheduleMessagePumpWork(const delay_ms : int64);
{$IFDEF MSWINDOWS}
var
  TempHandle : HWND;
{$ENDIF}
...
procedure TOldFMXWorkScheduler.StopScheduler;
begin
  FStopped := True;
  NextPulse(0);
  DepleteWork;
...
procedure TOldFMXWorkScheduler.Thread_OnPulse(Sender: TObject);
begin
  if not(FStopped) then DoMessageLoopWork;
end;

procedure TOldFMXWorkScheduler.DoWork;
begin
  DoMessageLoopWork;
  NextPulse(FDefaultInterval);
end;

procedure TOldFMXWorkScheduler.ScheduleWork(const delay_ms : int64);
begin
  if not(FStopped) then
    begin
      if (delay_ms <= 0) then
...
procedure TOldFMXWorkScheduler.NextPulse(aInterval : integer);
begin
