...
type
  TOldCefv8HandlerRef = class(TOldCefBaseRef, IOldCefv8Handler)
    protected
    ...
    public
      class function UnWrap(data: Pointer): IOldCefv8Handler;
  end;

  TOldCefv8HandlerOwn = class(TOldCefBaseOwn, IOldCefv8Handler)
    protected
      function Execute(const name: oldustring; const obj: IOldCefv8Value; const arguments: TOldCefv8ValueArray; var retval: IOldCefv8Value; var exception: oldustring): Boolean; virtual;
...
{$IFDEF DELPHI14_UP}
  TOldCefRTTIExtension = class(TOldCefv8HandlerOwn)
    protected
      FValue: TValue;
      FCtx: TRttiContext;
      FSyncMainThread: Boolean;

      function GetValue(pi: PTypeInfo; const v: IOldCefv8Value; var ret: TValue): Boolean;
      function SetValue(const v: TValue; var ret: IOldCefv8Value): Boolean;
  {$IFDEF CPUX64}
      class function StrToPtr(const str: oldustring): Pointer;
      class function PtrToStr(p: Pointer): oldustring;
  {$ENDIF}
...
uses
  oldCEFMiscFunctions, oldCEFLibFunctions, oldCEFv8Value, oldCEFConstants;

function cef_v8_handler_execute(      self           : POldCefv8Handler;
                                const name           : POldCefString;
                                      obj            : POldCefv8Value;
                                      argumentsCount : NativeUInt;
                                const arguments      : PPOldCefV8Value;
                                var   retval         : POldCefV8Value;
                                var   exception      : TOldCefString): Integer; stdcall;
var
  args : TOldCefv8ValueArray;
  i, j : NativeInt;
  ret  : IOldCefv8Value;
  exc  : oldustring;
  TempObject : TObject;
begin
  ...
  if (TempObject <> nil) and (TempObject is TOldCefv8HandlerOwn) then
    begin
      i := 0;
      j := argumentsCount;
      ...
      while (i < j) do
        begin
          args[i] := TOldCefv8ValueRef.UnWrap(arguments[i]);
          inc(i);
        end;

      Result    := -Ord(TOldCefv8HandlerOwn(TempObject).Execute(CefString(name), TOldCefv8ValueRef.UnWrap(obj), args, ret, exc));
...
begin
  ...
  ret       := nil;
  n         := CefString(name);
  Result    := POldCefv8Handler(FData).execute(POldCefv8Handler(FData), @n, CefGetData(obj), Length(arguments), @args, ret, exc) <> 0;
  retval    := TOldCefv8ValueRef.UnWrap(ret);
  exception := CefStringClearAndGet(exc);
end;

class function TOldCefv8HandlerRef.UnWrap(data: Pointer): IOldCefv8Handler;
begin
  if (data <> nil) then
    Result := Create(data) as IOldCefv8Handler
   else
    Result := nil;
end;
...
constructor TOldCefv8HandlerOwn.Create;
begin
  inherited CreateData(SizeOf(TOldCefv8Handler));

  POldCefv8Handler(FData).execute := cef_v8_handler_execute;
end;

function TOldCefv8HandlerOwn.Execute(const name: oldustring; const obj: IOldCefv8Value; const arguments: TOldCefv8ValueArray; var retval: IOldCefv8Value; var exception: oldustring): Boolean;
begin
  Result := False;
end;
...
{$IFDEF DELPHI14_UP}
// TOldCefRTTIExtension

constructor TOldCefRTTIExtension.Create(const value: TValue; SyncMainThread: Boolean);
begin
  inherited Create;
...
destructor TOldCefRTTIExtension.Destroy;
begin
  FCtx.Free;
...
function TOldCefRTTIExtension.GetValue(pi: PTypeInfo; const v: IOldCefv8Value; var ret: TValue): Boolean;
...
{$ENDIF}
  ...
  begin
    if pi = TypeInfo(IOldCefv8Value) then
    begin
...
function TOldCefRTTIExtension.SetValue(const v: TValue; var ret: IOldCefv8Value): Boolean;

  function ProcessRecord: Boolean;
  var
    rf: TRttiField;
    vl: TValue;
    ud, v8: IOldCefv8Value;
    rec: Pointer;
    rt: TRttiType;
  begin
    ud := TOldCefv8ValueRef.NewArray(1);
    rt := FCtx.GetType(v.TypeInfo);
{$IFDEF CPUX64}
    ud.SetValueByIndex(0, TOldCefv8ValueRef.NewString(PtrToStr(rt)));
{$ELSE}
    ud.SetValueByIndex(0, TOldCefv8ValueRef.NewInt(Integer(rt)));
{$ENDIF}
    ret := TOldCefv8ValueRef.NewObject(nil);
    ret.SetUserData(ud);
...
{$ENDIF}
  ...
  var
    ...
    fl: TRttiField;
    f: IOldCefv8Value;
    _r, _g, _s, ud: IOldCefv8Value;
    _a: TOldCefv8ValueArray;
    rt: TRttiType;
  begin
    rt := FCtx.GetType(v.TypeInfo);

    ud := TOldCefv8ValueRef.NewArray(2);
{$IFDEF CPUX64}
    ud.SetValueByIndex(0, TOldCefv8ValueRef.NewString(PtrToStr(rt)));
    ud.SetValueByIndex(1, TOldCefv8ValueRef.NewString(PtrToStr(v.AsObject)));
{$ELSE}
    ud.SetValueByIndex(0, TOldCefv8ValueRef.NewInt(Integer(rt)));
    ud.SetValueByIndex(1, TOldCefv8ValueRef.NewInt(Integer(v.AsObject)));
{$ENDIF}
    ret := TOldCefv8ValueRef.NewObject(nil); // todo
    ret.SetUserData(ud);

    for m in rt.GetMethods do
      if m.Visibility > mvProtected then
      begin
        f := TOldCefv8ValueRef.NewFunction(m.Name, Self);
    ...
    for p in rt.GetProperties do
      ...
      begin
        ...
        SetLength(_a, 2);
        _a[0] := TOldCefv8ValueRef.NewString(p.Name);
        if p.IsReadable then
        begin
          _a[1] := TOldCefv8ValueRef.NewFunction('$pg' + p.Name, Self);
          _r := _g.ExecuteFunction(ret, _a);
        end;
        if p.IsWritable then
        begin
          _a[1] := TOldCefv8ValueRef.NewFunction('$ps' + p.Name, Self);
    ...
    for fl in rt.GetFields do
      ...
      begin
        ...
        SetLength(_a, 2);
        _a[0] := TOldCefv8ValueRef.NewString(fl.Name);
        _a[1] := TOldCefv8ValueRef.NewFunction('$vg' + fl.Name, Self);
        _r := _g.ExecuteFunction(ret, _a);
        _a[1] := TOldCefv8ValueRef.NewFunction('$vs' + fl.Name, Self);
        _r := _s.ExecuteFunction(ret, _a);
      end;
...
{$IFDEF CPUX64}
    ud.SetValueByIndex(0, TOldCefv8ValueRef.NewString(PtrToStr(rt)));
    ud.SetValueByIndex(1, TOldCefv8ValueRef.NewString(PtrToStr(c)));
{$ELSE}
    ud.SetValueByIndex(0, TOldCefv8ValueRef.NewInt(Integer(rt)));
    ud.SetValueByIndex(1, TOldCefv8ValueRef.NewInt(Integer(c)));
{$ENDIF}
    ret := TOldCefv8ValueRef.NewObject(nil); // todo
    ret.SetUserData(ud);
  ...
  begin
    ...
    case TVarData(vr).VType of
      varSmallint, varInteger, varShortInt:
        ret := TOldCefv8ValueRef.NewInt(vr);
      varByte, varWord, varLongWord:
        ret := TOldCefv8ValueRef.NewUInt(vr);
      varUString, varOleStr, varString:
        ret := TOldCefv8ValueRef.NewString(vr);
      varSingle, varDouble, varCurrency, varUInt64, varInt64:
        ret := TOldCefv8ValueRef.NewDouble(vr);
      varBoolean:
        ret := TOldCefv8ValueRef.NewBool(vr);
      varNull:
        ret := TOldCefv8ValueRef.NewNull;
      varEmpty:
        ret := TOldCefv8ValueRef.NewUndefined;
    else
      ret := nil;
      Exit(False)
    end;
  ...
  begin
    ...
    if TypeInfo(IOldCefv8Value) = v.TypeInfo then
    begin
      ret := IOldCefv8Value(v.AsInterface);
      Result := True;
    end else
    begin
      rt := FCtx.GetType(v.TypeInfo);
  ...
  {$IFDEF CPUX64}
      ud.SetValueByIndex(0, TOldCefv8ValueRef.NewString(PtrToStr(rt)));
      ud.SetValueByIndex(1, TOldCefv8ValueRef.NewString(PtrToStr(Pointer(v.AsInterface))));
  {$ELSE}
      ud.SetValueByIndex(0, TOldCefv8ValueRef.NewInt(Integer(rt)));
      ud.SetValueByIndex(1, TOldCefv8ValueRef.NewInt(Integer(v.AsInterface)));
  {$ENDIF}
      ret := TOldCefv8ValueRef.NewObject(nil);
      ret.SetUserData(ud);
  ...
  begin
    if v.TypeInfo = TypeInfo(TDateTime) then
      ret := TOldCefv8ValueRef.NewDate(TValueData(v).FAsDouble) else
      ret := TOldCefv8ValueRef.NewDouble(v.AsExtended);
    Result := True;
  end;
...
begin
  case v.TypeInfo.Kind of
    tkUString, tkLString, tkWString, tkChar, tkWChar:
      ret := TOldCefv8ValueRef.NewString(v.AsString);
    tkInteger: ret := TOldCefv8ValueRef.NewInt(v.AsInteger);
    tkEnumeration:
      if v.TypeInfo = TypeInfo(Boolean) then
        ret := TOldCefv8ValueRef.NewBool(v.AsBoolean) else
        ret := TOldCefv8ValueRef.NewInt(TValueData(v).FAsSLong);
    tkFloat: if not ProcessFloat then Exit(False);
    tkInt64: ret := TOldCefv8ValueRef.NewDouble(v.AsInt64);
...
class procedure TOldCefRTTIExtension.Register(const name: string; const value: TValue; SyncMainThread: Boolean);
var
  TempCode    : oldustring;
  TempHandler : IOldCefv8Handler;
begin
  TempHandler := TOldCefRTTIExtension.Create(value, SyncMainThread);
...
{$IFDEF CPUX64}
class function TOldCefRTTIExtension.StrToPtr(const str: oldustring): Pointer;
begin
  HexToBin(PWideChar(str), @Result, SizeOf(Result));
end;

class function TOldCefRTTIExtension.PtrToStr(p: Pointer): oldustring;
begin
...
function TOldCefRTTIExtension.Execute(const name: oldustring; const obj: IOldCefv8Value;
  const arguments: TOldCefv8ValueArray; var retval: IOldCefv8Value;
  var exception: oldustring): Boolean;
var
  p: PChar;
  ud: IOldCefv8Value;
