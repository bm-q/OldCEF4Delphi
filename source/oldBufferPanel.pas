...
type
  ...
  TOldBufferPanel = class(TCustomPanel)
    protected
      FMutex          : THandle;
      FBuffer         : TBitmap;
...
constructor TOldBufferPanel.Create(AOwner: TComponent);
begin
...
destructor TOldBufferPanel.Destroy;
begin
  DestroyBuffer;
  DestroySyncObj;
...
procedure TOldBufferPanel.AfterConstruction;
begin
...
procedure TOldBufferPanel.CreateSyncObj;
begin
  FMutex := CreateMutex(nil, False, nil);
end;

procedure TOldBufferPanel.DestroySyncObj;
begin
  if (FMutex <> 0) then
    begin
      CloseHandle(FMutex);
...
procedure TOldBufferPanel.DestroyBuffer;
begin
...
function TOldBufferPanel.SaveBufferToFile(const aFilename : string) : boolean;
begin
  Result := False;
...
function TOldBufferPanel.SaveToFile(const aFilename : string) : boolean;
begin
  Result := False;
...
function TOldBufferPanel.InvalidatePanel : boolean;
begin
  Result := HandleAllocated and PostMessage(Handle, CM_INVALIDATE, 0, 0);
end;

function TOldBufferPanel.BeginBufferDraw : boolean;
begin
  Result := (FMutex <> 0) and (WaitForSingleObject(FMutex, 5000) = WAIT_OBJECT_0);
end;

procedure TOldBufferPanel.EndBufferDraw;
begin
  if (FMutex <> 0) then ReleaseMutex(FMutex);
end;

function TOldBufferPanel.CopyBuffer : boolean;
begin
  Result := False;
...
procedure TOldBufferPanel.Paint;
begin
...
procedure TOldBufferPanel.WMEraseBkgnd(var aMessage : TWMEraseBkgnd);
begin
  aMessage.Result := 1;
end;

function TOldBufferPanel.GetBufferBits : pointer;
begin
  if (FBuffer <> nil) then
...
function TOldBufferPanel.GetBufferWidth : integer;
begin
  if (FBuffer <> nil) then
...
function TOldBufferPanel.GetBufferHeight : integer;
begin
  if (FBuffer <> nil) then
...
procedure TOldBufferPanel.BufferDraw(x, y : integer; const aBitmap : TBitmap);
begin
  if (FBuffer <> nil) then FBuffer.Canvas.Draw(x, y, aBitmap);
end;

function TOldBufferPanel.UpdateBufferDimensions(aWidth, aHeight : integer) : boolean;
begin
  Result := False;
...
function TOldBufferPanel.BufferIsResized(aUseMutex : boolean) : boolean;
var
...
begin
  ...
  if not(aUseMutex) or BeginBufferDraw then
    begin
      if (GlobalOldCEFApp.DeviceScaleFactor = 1) then
        begin
          Result := (FBuffer <> nil) and
        ...
        begin
          ...
          TempLogWidth  := DeviceToLogical(Width,  GlobalOldCEFApp.DeviceScaleFactor);
          TempLogHeight := DeviceToLogical(Height, GlobalOldCEFApp.DeviceScaleFactor);

          TempDevWidth  := LogicalToDevice(TempLogWidth,  GlobalOldCEFApp.DeviceScaleFactor);
          TempDevHeight := LogicalToDevice(TempLogHeight, GlobalOldCEFApp.DeviceScaleFactor);
