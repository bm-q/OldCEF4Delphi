...
uses
  ...
  {$ENDIF}
  oldCEFConstants, oldCEFWorkSchedulerThread;

type
  {$IFNDEF FPC}{$IFDEF DELPHI16_UP}[ComponentPlatformsAttribute(pidWin32 or pidWin64)]{$ENDIF}{$ENDIF}
  TOldCEFWorkScheduler = class(TComponent)
    protected
      FCompHandle         : HWND;
      FThread             : TOldCefWorkSchedulerThread;
...
var
  GlobalOldCEFWorkScheduler : TOldCEFWorkScheduler = nil;

procedure DestroyGlobalOldCEFWorkScheduler;
...
procedure DestroyGlobalOldCEFWorkScheduler;
begin
  if (GlobalOldCEFWorkScheduler <> nil) then FreeAndNil(GlobalOldCEFWorkScheduler);
end;

constructor TOldCEFWorkScheduler.Create(AOwner: TComponent);
begin
...
destructor TOldCEFWorkScheduler.Destroy;
begin
  DestroyThread;
...
procedure TOldCEFWorkScheduler.AfterConstruction;
begin
...
procedure TOldCEFWorkScheduler.CreateThread;
begin
  FThread                 := TOldCefWorkSchedulerThread.Create;
  {$IFDEF MSWINDOWS}
...
procedure TOldCEFWorkScheduler.DestroyThread;
begin
  try
    if (FThread <> nil) then
      begin
    ...
    on e : exception do
      if CustomExceptionHandler('TOldCefWorkScheduler.DestroyThread', e) then raise;
  end;
end;

procedure TOldCEFWorkScheduler.WndProc(var aMessage: TMessage);
begin
...
procedure TOldCEFWorkScheduler.DeallocateWindowHandle;
begin
...
procedure TOldCEFWorkScheduler.DoMessageLoopWork;
begin
  if (GlobalOldCEFApp <> nil) then GlobalOldCEFApp.DoMessageLoopWork;
end;

procedure TOldCEFWorkScheduler.SetDefaultInterval(aValue : integer);
begin
...
{$IFDEF MSWINDOWS}
{$WARN SYMBOL_PLATFORM OFF}
procedure TOldCEFWorkScheduler.SetPriority(aValue : TThreadPriority);
begin
  FPriority := aValue;
...
procedure TOldCEFWorkScheduler.DepleteWork;
var
  i : cardinal;
begin
...
procedure TOldCEFWorkScheduler.ScheduleMessagePumpWork(const delay_ms : int64);
begin
  if not(FStopped) and (FCompHandle <> 0) then
    PostMessage(FCompHandle, CEF_PUMPHAVEWORK, 0, LPARAM(delay_ms));
end;

procedure TOldCEFWorkScheduler.StopScheduler;
begin
  FStopped := True;
  NextPulse(0);
  DepleteWork;
...
procedure TOldCEFWorkScheduler.Thread_OnPulse(Sender: TObject);
begin
  if not(FStopped) then DoMessageLoopWork;
end;

procedure TOldCEFWorkScheduler.DoWork;
begin
  DoMessageLoopWork;
  NextPulse(FDefaultInterval);
end;

procedure TOldCEFWorkScheduler.ScheduleWork(const delay_ms : int64);
begin
...
procedure TOldCEFWorkScheduler.NextPulse(aInterval : integer);
begin
