...
uses
  ...
  {$ENDIF}
  oldCEFTypes, oldCEFInterfaces, oldCEFLibFunctions, oldCEFResourceHandler, oldCEFConstants;

const
  Kernel32DLL = 'kernel32.dll';
  SHLWAPIDLL  = 'shlwapi.dll';

function CefColorGetA(color: TOldCefColor): Byte;
function CefColorGetR(color: TOldCefColor): byte;
function CefColorGetG(color: TOldCefColor): Byte;
function CefColorGetB(color: TOldCefColor): Byte;

function CefColorSetARGB(a, r, g, b: Byte): TOldCefColor;
...
function CefGetData(const i: IOldCefBase): Pointer; {$IFDEF SUPPORTS_INLINE} inline; {$ENDIF}

function CefStringAlloc(const str: oldustring): TOldCefString;
function CefStringClearAndGet(var str: TOldCefString): oldustring;

function  CefString(const str: oldustring): TOldCefString; overload;
function  CefString(const str: POldCefString): oldustring; overload;
function  CefUserFreeString(const str: oldustring): POldCefStringUserFree;
procedure CefStringFree(const str: POldCefString);
function  CefStringFreeAndGet(const str: POldCefStringUserFree): oldustring;
procedure CefStringSet(const str: POldCefString; const value: oldustring);

function  CefExecuteProcess(var app : IOldCefApp; aWindowsSandboxInfo : Pointer = nil) : integer;
function  CefRegisterExtension(const name, code: oldustring; const Handler: IOldCefv8Handler): Boolean;
procedure CefPostTask(ThreadId: TOldCefThreadId; const task: IOldCefTask);
procedure CefPostDelayedTask(ThreadId: TOldCefThreadId; const task: IOldCefTask; delayMs: Int64);

function CefTimeToSystemTime(const dt: TOldCefTime): TSystemTime;
function SystemTimeToCefTime(const dt: TSystemTime): TOldCefTime;
function CefTimeToDateTime(const dt: TOldCefTime): TDateTime;
function DateTimeToCefTime(dt: TDateTime): TOldCefTime;

function cef_string_wide_copy(const src: PWideChar; src_len: NativeUInt;  output: POldCefStringWide): Integer;
function cef_string_utf8_copy(const src: PAnsiChar; src_len: NativeUInt; output: POldCefStringUtf8): Integer;
function cef_string_utf16_copy(const src: OldPChar16; src_len: NativeUInt; output: POldCefStringUtf16): Integer;
function cef_string_copy(const src: POldCefChar; src_len: NativeUInt; output: POldCefString): Integer;

procedure WindowInfoAsChild(var aWindowInfo : TOldCefWindowInfo; aParent : THandle; aRect : TRect; const aWindowName : string = '');
procedure WindowInfoAsPopUp(var aWindowInfo : TOldCefWindowInfo; aParent : THandle; const aWindowName : string = '');
procedure WindowInfoAsWindowless(var aWindowInfo : TOldCefWindowInfo; aParent : THandle; const aWindowName : string = '');
...
function GetGlobalMemoryStatusEx(var Buffer: TMyOldMemoryStatusEx): BOOL; stdcall; external Kernel32DLL name 'GlobalMemoryStatusEx';
...
function CefRegisterSchemeHandlerFactory(const SchemeName, DomainName : oldustring; const handler: TOldCefResourceHandlerClass = nil): Boolean;
function CefClearSchemeHandlerFactories : boolean;

function CefAddCrossOriginWhitelistEntry(const SourceOrigin, TargetProtocol, TargetDomain: oldustring; AllowTargetSubdomains: Boolean): Boolean;
function CefRemoveCrossOriginWhitelistEntry(const SourceOrigin, TargetProtocol, TargetDomain: oldustring; AllowTargetSubdomains: Boolean): Boolean;
function CefClearCrossOriginWhitelist: Boolean;

procedure UInt64ToFileVersionInfo(const aVersion : uint64; var aVersionInfo : TOldFileVersionInfo);
...
function  GetDLLVersion(const aDLLFile : string; var aVersionInfo : TOldFileVersionInfo) : boolean;
...
function FileVersionInfoToString(const aVersionInfo : TOldFileVersionInfo) : string;
...
function  CefParseUrl(const url: oldustring; var parts: TOldUrlParts): Boolean;
function  CefCreateUrl(var parts: TOldUrlParts): oldustring;
function  CefFormatUrlForSecurityDisplay(const originUrl, languages : string): string;
function  CefGetMimeType(const extension: oldustring): oldustring;
procedure CefGetExtensionsForMimeType(const mimeType: oldustring; var extensions: TStringList);

function CefBase64Encode(const data: Pointer; dataSize: NativeUInt): oldustring;
function CefBase64Decode(const data: oldustring): IOldCefBinaryValue;
function CefUriEncode(const text: oldustring; usePlus: Boolean): oldustring;
function CefUriDecode(const text: oldustring; convertToUtf8: Boolean; unescapeRule: TOldCefUriUnescapeRule): oldustring;

function CefParseJson(const jsonString: oldustring; options: TOldCefJsonParserOptions): IOldCefValue;
function CefParseJsonAndReturnError(const jsonString   : oldustring;
                                          options      : TOldCefJsonParserOptions;
                                    out   errorCodeOut : TOldCefJsonParserError;
                                    out   errorMsgOut  : oldustring): IOldCefValue;
function CefWriteJson(const node: IOldCefValue; options: TOldCefJsonWriterOptions): oldustring;

function CefIsKeyDown(aWparam : WPARAM) : boolean;
function CefIsKeyToggled(aWparam : WPARAM) : boolean;
function GeTOldCefMouseModifiers(awparam : WPARAM) : TOldCefEventFlags; overload;
function GeTOldCefMouseModifiers : TOldCefEventFlags; overload;
function GeTOldCefKeyboardModifiers(aWparam : WPARAM; aLparam : LPARAM) : TOldCefEventFlags;
function GefCursorToWindowsCursor(aCefCursor : TOldCefCursorType) : TCursor;

procedure DropEffectToDragOperation(aEffect : Longint; var aAllowedOps : TOldCefDragOperations);
procedure DragOperationToDropEffect(const aDragOperations : TOldCefDragOperations; var aEffect: Longint);

function  DeviceToLogical(aValue : integer; const aDeviceScaleFactor : double) : integer; overload;
procedure DeviceToLogical(var aEvent : TOldCefMouseEvent; const aDeviceScaleFactor : double); overload;
...
procedure LogicalToDevice(var aRect : TOldCefRect; const aDeviceScaleFactor : double); overload;
...
uses
  oldCEFApplication, oldCEFSchemeHandlerFactory, oldCEFValue,
  oldCEFBinaryValue, oldCEFStringList;
...
function CefString(const str: POldCefString): oldustring;
begin
  if (str <> nil) then
...
function CefString(const str: oldustring): TOldCefString;
begin
  Result.str    := OldPChar16(PWideChar(str));
...
begin
  if (str <> nil) then
    begin
      Result := CefString(POldCefString(str));
...
procedure _free_string(str: OldPChar16); stdcall;
begin
...
begin
  ...
  Result.length := Length(str);
  GetMem(Result.str, Result.length * SizeOf(TOldCefChar));
  Move(POldCefChar(str)^, Result.str^, Result.length * SizeOf(TOldCefChar));
  Result.dtor   := @_free_string;
end;

function CefExecuteProcess(var app : IOldCefApp; aWindowsSandboxInfo : Pointer) : integer;
begin
...
var
  st: TSystemTime;
begin
  FillChar(Result, SizeOf(TOldCefTime), 0);
...
var
  TempString : string;
begin
  if (GlobalOldCEFApp <> nil) and GlobalOldCEFApp.LibLoaded then
    begin
      TempString := 'PID: ' + IntToStr(GetCurrentProcessID) + ', TID: ' + IntToStr(GetCurrentThreadID);

      case GlobalOldCEFApp.ProcessType of
...
begin
  ...
  Result := (GlobalOldCEFApp <> nil) and GlobalOldCEFApp.ReRaiseExceptions;
end;

function CefRegisterSchemeHandlerFactory(const SchemeName : oldustring;
                                         const DomainName : oldustring;
                                         const handler    : TOldCefResourceHandlerClass) : boolean;
var
  TempScheme, TempDomainName : TOldCefString;
  TempFactory : IOldCefSchemeHandlerFactory;
  TempDomainNamePtr : POldCefString;
begin
  Result := False;

  try
    if (GlobalOldCEFApp <> nil)    and
       GlobalOldCEFApp.LibLoaded   and
      ...
      begin
        ...
        TempFactory := TOldCefSchemeHandlerFactoryOwn.Create(handler);
...
function CefClearSchemeHandlerFactories : boolean;
begin
  Result := (GlobalOldCEFApp <> nil)  and
            GlobalOldCEFApp.LibLoaded and
            (cef_clear_scheme_handler_factories() <> 0);
end;

function CefAddCrossOriginWhitelistEntry(const SourceOrigin          : oldustring;
                                         const TargetProtocol        : oldustring;
                                         const TargetDomain          : oldustring;
                                               AllowTargetSubdomains : Boolean): Boolean;
var
  TempSourceOrigin, TempTargetProtocol, TempTargetDomain : TOldCefString;
begin
...
function CefRemoveCrossOriginWhitelistEntry(const SourceOrigin          : oldustring;
                                            const TargetProtocol        : oldustring;
                                            const TargetDomain          : oldustring;
                                                  AllowTargetSubdomains : Boolean): Boolean;
var
...
begin
  s := CefString(data);
  Result := TOldCefBinaryValueRef.UnWrap(cef_base64decode(@s));
end;
...
function CefUriDecode(const text: oldustring; convertToUtf8: Boolean;
  unescapeRule: TOldCefUriUnescapeRule): oldustring;
var
  s: TOldCefString;
begin
  s := CefString(text);
...
begin
  ...
  Result := TOldCefValueRef.UnWrap(cef_parse_json(@s, options));
end;
...
begin
  ...
  Result := TOldCefValueRef.UnWrap(cef_parse_jsonand_return_error(@s, options, @errorCodeOut, @e));
...
function GeTOldCefMouseModifiers(aWparam : WPARAM) : TOldCefEventFlags;
begin
...
function GeTOldCefMouseModifiers : TOldCefEventFlags;
begin
  Result := EVENTFLAG_NONE;
...
procedure DeviceToLogical(var aEvent : TOldCefMouseEvent; const aDeviceScaleFactor : double);
begin
...
procedure LogicalToDevice(var aRect : TOldCefRect; const aDeviceScaleFactor : double);
begin
