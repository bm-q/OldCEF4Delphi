...
type
  ...
  TOldFMXBufferPanel = class(TControl)
    protected
      {$IFDEF MSWINDOWS}
      FMutex            : THandle;
      {$ELSE}
...
uses
  ...
  FMX.Platform, oldCEFMiscFunctions, oldCEFApplication;

constructor TOldFMXBufferPanel.Create(AOwner: TComponent);
begin
...
destructor TOldFMXBufferPanel.Destroy;
begin
  DestroyBuffer;
  DestroySyncObj;
...
procedure TOldFMXBufferPanel.AfterConstruction;
begin
...
procedure TOldFMXBufferPanel.CreateSyncObj;
begin
  {$IFDEF MSWINDOWS}
...
procedure TOldFMXBufferPanel.DestroySyncObj;
begin
  {$IFDEF MSWINDOWS}
  if (FMutex <> 0) then
    begin
...
procedure TOldFMXBufferPanel.DestroyBuffer;
begin
...
function TOldFMXBufferPanel.SaveBufferToFile(const aFilename : string) : boolean;
begin
  Result := False;
...
function TOldFMXBufferPanel.SaveToFile(const aFilename : string) : boolean;
begin
  Result := False;
...
procedure TOldFMXBufferPanel.InvalidatePanel;
begin
  InvalidateRect(TRectF.Create(0, 0, Width, Height));
end;

function TOldFMXBufferPanel.BeginBufferDraw : boolean;
begin
  {$IFDEF MSWINDOWS}
...
procedure TOldFMXBufferPanel.EndBufferDraw;
begin
  {$IFDEF MSWINDOWS}
...
function TOldFMXBufferPanel.CopyBuffer : boolean;
var
...
procedure TOldFMXBufferPanel.DialogKey(var Key: Word; Shift: TShiftState);
begin
...
procedure TOldFMXBufferPanel.Paint;
var
  TempRect : TRectF;
begin
...
function TOldFMXBufferPanel.GetScreenScale : Single;
begin
  if (GlobalOldCEFApp <> nil) then
    Result := GlobalOldCEFApp.DeviceScaleFactor
   else
    Result := 1;
end;

function TOldFMXBufferPanel.GetBufferWidth : integer;
begin
  if (FBuffer <> nil) then
...
function TOldFMXBufferPanel.GetBufferHeight : integer;
begin
  if (FBuffer <> nil) then
...
procedure TOldFMXBufferPanel.BufferDraw(x, y : integer; const aBitmap : TBitmap);
var
  TempSrc, TempDst : TRectF;
begin
  if (FBuffer <> nil) then
...
function TOldFMXBufferPanel.UpdateBufferDimensions(aWidth, aHeight : integer) : boolean;
begin
  Result := False;
...
function TOldFMXBufferPanel.BufferIsResized(aUseMutex : boolean) : boolean;
var
...
begin
  ...
  if not(aUseMutex) or BeginBufferDraw then
    begin
      TempWidth  := round(Width  * GlobalOldCEFApp.DeviceScaleFactor);
      TempHeight := round(Height * GlobalOldCEFApp.DeviceScaleFactor);
...
function TOldFMXBufferPanel.ScreenToClient(aPoint : TPoint) : TPoint;
var
  TempPoint : TPointF;
begin
...
function TOldFMXBufferPanel.ClientToScreen(aPoint : TPoint) : TPoint;
var
  TempPoint : TPointF;
begin
  TempPoint.x := aPoint.x;
