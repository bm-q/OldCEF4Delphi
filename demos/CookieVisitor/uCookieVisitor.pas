...
uses
  ...
  oldCEFCookieManager, oldCEFCookieVisitor;
...
const
  ...
  MINIBROWSER_CONTEXTMENU_GETOldCookieS    = MENU_ID_USER_FIRST + 2;
  MINIBROWSER_CONTEXTMENU_SetCookie     = MENU_ID_USER_FIRST + 3;

type
  TOldCookieVisitorFrm = class(TForm)
  ...
  protected
    FText     : string;
    FVisitor  : IOldCefCookieVisitor;
    // Variables to control when can we destroy the form safely
    FCanClose : boolean;  // Set to True in TOldChromium.OnBeforeClose
    FClosing  : boolean;  // Set to True in the CloseQuery event.

  public
    procedure AddCookieInfo(const aCookie : TOldCookie);

  end;

var
  CookieVisitorFrm: TOldCookieVisitorFrm;
...
function CookieVisitorProc(const name, value, domain, path: oldustring;
                                 secure, httponly, hasExpires: Boolean;
                           const creation, lastAccess, expires: TDateTime;
                                 count, total: Integer;
                           out   deleteCookie: Boolean): Boolean;
var
  TempCookie : TOldCookie;
begin
  deleteCookie := False;
...
procedure TOldCookieVisitorFrm.AddCookieInfo(const aCookie : TOldCookie);
begin
  // This should be protected by a mutex.
  FText := FText + aCookie.name + ' : ' + aCookie.value + #13 + #10;
end;

procedure TOldCookieVisitorFrm.BrowserCreatedMsg(var aMessage : TMessage);
begin
...
procedure TOldCookieVisitorFrm.BrowserDestroyMsg(var aMessage : TMessage);
begin
  CEFWindowParent1.Free;
end;

procedure TOldCookieVisitorFrm.ShowCookiesMsg(var aMessage : TMessage);
begin
  SimpleTextViewerFrm.Memo1.Lines.Text := FText; // This should be protected by a mutex.
  SimpleTextViewerFrm.ShowModal;
end;

procedure TOldCookieVisitorFrm.SetCookieRsltMsg(var aMessage : TMessage);
begin
...
procedure TOldCookieVisitorFrm.Timer1Timer(Sender: TObject);
begin
...
procedure TOldCookieVisitorFrm.GoBtnClick(Sender: TObject);
begin
  Chromium1.LoadURL(Edit1.Text);
end;

procedure TOldCookieVisitorFrm.Chromium1AfterCreated(Sender: TObject; const browser: IOldCefBrowser);
begin
  PostMessage(Handle, CEF_AFTERCREATED, 0, 0);
end;

procedure TOldCookieVisitorFrm.Chromium1BeforeClose(Sender: TObject; const browser: IOldCefBrowser);
begin
  FCanClose := True;
  PostMessage(Handle, WM_CLOSE, 0, 0);
end;

procedure TOldCookieVisitorFrm.Chromium1BeforeContextMenu(Sender: TObject;
  const browser: IOldCefBrowser; const frame: IOldCefFrame;
  const params: IOldCefContextMenuParams; const model: IOldCefMenuModel);
begin
  model.AddSeparator;
  ...
  model.AddSeparator;
  model.AddItem(MINIBROWSER_CONTEXTMENU_GETOldCookieS,     'Visit cookies');
  model.AddItem(MINIBROWSER_CONTEXTMENU_SetCookie,      'Set cookie');
end;

procedure TOldCookieVisitorFrm.Chromium1BeforePopup(Sender: TObject;
  const browser: IOldCefBrowser; const frame: IOldCefFrame; const targetUrl,
...
procedure TOldCookieVisitorFrm.Chromium1Close(Sender: TObject;
...
procedure TOldCookieVisitorFrm.Chromium1ContextMenuCommand(Sender: TObject;
  const browser: IOldCefBrowser; const frame: IOldCefFrame;
  const params: IOldCefContextMenuParams; commandId: Integer;
  eventFlags: Cardinal; out Result: Boolean);
var
...
begin
  ...
  case commandId of
    ...
    MINIBROWSER_CONTEXTMENU_GETOldCookieS :
      begin
        // This should be protected by a mutex
        FText       := '';
        TempManager := TOldCefCookieManagerRef.Global(nil);
...
procedure TOldCookieVisitorFrm.Chromium1CookiesDeleted(Sender: TObject; numDeleted: Integer);
begin
  showmessage('Deleted cookies : ' + inttostr(numDeleted));
end;

procedure TOldCookieVisitorFrm.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := FCanClose;
...
procedure TOldCookieVisitorFrm.FormCreate(Sender: TObject);
begin
  FVisitor  := TOldCefFastCookieVisitor.Create(CookieVisitorProc);
  FCanClose := False;
  FClosing  := False;
end;

procedure TOldCookieVisitorFrm.FormDestroy(Sender: TObject);
begin
  FVisitor := nil;
end;

procedure TOldCookieVisitorFrm.FormShow(Sender: TObject);
begin
...
procedure TOldCookieVisitorFrm.WMMove(var aMessage : TWMMove);
begin
  inherited;
...
procedure TOldCookieVisitorFrm.WMMoving(var aMessage : TMessage);
begin
  inherited;
...
procedure TOldCookieVisitorFrm.WMEnterMenuLoop(var aMessage: TMessage);
begin
  inherited;
...
procedure TOldCookieVisitorFrm.WMExitMenuLoop(var aMessage: TMessage);
begin
  inherited;
